...
jl = select(Address).options(joinedload(Address.user, innerjoin = True)).order_by(Address.id)

for user_obj in session.execute(jl):
    print(user_obj.Address.email, user_obj.Address.user.name)